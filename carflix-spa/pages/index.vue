            ...
            <td>
              {{
                endereco.dataCriacao
                  ? $moment(endereco.dataCriacao).format("DD/MM/YYYY")
                  : ""
              }}
            </td>
            <td>
  ...
  methods: {
    ...
    async excluir(endereco) {
      const confirmed = await this.$bvModal.msgBoxConfirm(
        `Tem certeza que deseja excluir o CEP: ${endereco.cep}?`
      );

      if (!confirmed) return;

      const { enderecoId } = await this.$axios.$delete(
